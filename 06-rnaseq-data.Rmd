...
ggplot(summary_df, aes(x = timepoint, y = mean_counts)) +
  geom_line(aes(group = gene_name), alpha = 0.1) +
  scale_y_log10() + 
  facet_wrap(~cluster)
```
...
What about the gene from each cluster with the highest expression at this timepoint? Using the `group_by` function before
 `top_n` will return the top genes from each group.
