...
The first row has a NA value for clade, so if we wanted to remove those we could
insert a `filter()` in this chain, using a built-in R function to identify NA values:
...
![Split apply combine](./img/12-plyr-fig1.png)
...
For example, there are built-in functions in R like
